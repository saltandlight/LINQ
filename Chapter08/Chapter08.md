...
- LINQ to SQL이 데이터 접근방식을 수정할 수 있도록 제공하는 몇 가지 부가적인 옵션들에 대해 알아볼 것
- 각각의 경우 이 프로그래밍 모델은 사용자가 작성해야 하는 지루한 막노동같은 코드의 양을 대폭 줄여줌
- 직접 DB에 명령들을 수행시키는 방식을 확인한 후 어떻게 SQL Server의 저장된 프로시저나 사용자 정의 함수같은 고급옵션들을 사용할 수 있는지 알아보기

### 8.2.1 SQL 전달 : SQL 질의에서 객체를 반환하기
- LINQ의 질의기능은 데이터를 다루는 매우 혁신적인 방법을 제공하지만 아주 심각한 단점을 가지고 있음
- 객체를 기반으로 한 질의구조는 항상 컴파일 과정을 필요로 함 
- ad-hoc(임기응변식) 보고서나 다른 사용자 정의 데이터 접근상황에서는 종종 매우 유연한 모델이 필요함
- 이런 요구사항에 맞추기 위해서 LINQ to SQL은 동적인 SQL질의를 컴파일 과정 안 거치고 사용자가 직접 입력한 질의를 데이터베이스에 바로 보내는 기능을 제공함
- 이 기능을 사용하기 위해서는 DataContext 객체가 있어야 함
- 이 객체를 이용하여 수행하려는 SQL 명령문을 문자열 형태로 ExecuteQuery 문을 통해 수행시키면 됨
- [동적인 SQL 전달(pass-through, 전달형)]
```C#
string searchName;
...
```
- 표준적인 ADO.NET에 비해 데이터에 접근하기 위해 작성하는 코드의 양은 확연히 줄어듬
- ExecuteQuery 메소드를 사용 시, 데이터의 출처는 그닥 안 중요
- 중요한 것:**select문에서 반환된 열의 이름이 클래스 프로펕의 이름과 맞아 떨어져야 함**
- 이 값들이 맞아떨어지기만 한다면, 전달형(pass-through) 질의는 개발자가 지정하는, 엄격하게 형을 가지는 객체들을 반환 가능함

- 전달형 질의를 이용하는 경우, 질의 자체는 엄격하지 않은 형을 가지고 있음
- -> 사용자가 제공한 값들을 실제로 사용 가능한가? 신중하게 확인해야 함
- 그리고 문법들도 체크해야 함
- searchName에 이상한 값을 넣으면 예외가 발생할텐데 그것보다 무서운 것은 **SQL 인젝션을 이용한 공격**임
![](cap2.PNG)
출처: https://namu.wiki/w/SQL%20injection

- 만약 만약에... 사용자가 Good' OR ''=''라고 입력했다면... 성이 "Good"인 작가들으 ㅣ작품들이 모두 반환될 거고 빈 문자열이 빈 문자열과 같다면 사용자가 필요로 하지 않는 작가들까지 다 반환됨...

- 이에 대한 대안으로 전달될 질의는 String.Format 메소드가 사용하는 동일한 골뱅이 표시(@)를 이용하여 작성한 후 매개변수의 목록을 첨부하여 완성 가능함
- 매개변수로 값들이 전달됨
- DataContext는 표현식을 매개변수화된 질의로 변환시킴
- 이렇게 하면 SQL 인젝션 공격을 시도하는 사용자들이 당황할 것임
- [매개변수를 가진 동적인 SQL 전달형 질의]
```C#
...
```
- 생성된 SQL을 살펴보면 모든 질의가 매개변수화된 것을 알 수 있음(SQL 인젝션 공격을 피하기 위함)
```sql
Select ID, LastName, FirstName, WebSite, TimeStamp
From dbo.Author
Where LastName = @p0
...
- 동적 SQL의 단점: SQL 인젝션, 적절하지 못한 인덱싱 -> 성능이 낮은 질의가 될 수 있음
- 대부분의 DBA들은 동적인 SQL의 과도한 사용에 극구 반대할 것임(필요하기는 하지만...)

### 8.2.2 저장된 프로시저와 작업하기
- 동적인 SQL의 반대쪽 극단: SQL Server에 포함된 미리 컴파일된 저장된 프로시저
- 저장된 프로시저에 의존하는 가장 일반적인 이유: 보안, 성능, 관찰 등의 기능
- 성능의 관점: 매개변수화된 질의의 수행계획은 한 번 해석된 후 캐시에 저장된 프로시저처럼 캐시됨
- 보안상의 관점: 매개변수화된 질의는 SQL 인젝션에 의한 공격의 가능성을 없애줌

- LINQ to SQL은 테이블 수준에서 서버 사용권한을 필요로 함
- DBA에 따라 그것을 쉽게 허용하지 않는 경우가 있음 
- 저장된 프로시저는 DBA에게 데이터 접근을 제어하고 인덱스 스키마를 알맞게 수정하도록 해줌
- 그러나 저장된 프로시저를 사용 시, 단순히 DataContext에 의존해서 동적으로 CRUD SQL문들을 생성하고 업데이트가 필요한 프로퍼티들의 수를 제한할 수는 없음
- 전체 객체가 매번 업데이트되어야 함
- 그런 의미에서 만약 애플리케이션이 저장된 프로시저를 필요로 한다면 해당 프로시저를 이용하는 것이 상대적으로 쉬움

#### 저장된 프로시저로 데이터를 읽어들이기
#### 저장된 프로시저를 이용하여 업데이트하기
